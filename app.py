...
# https://drive.google.com/file/d/1MCtuDcQCTcTUakiGMaYA6GTnuWGH7GTE/view?usp=sharing
...
def read_and_load_data(file_path):
    return pd.read_csv(file_path)

# Load datasets
train_data = read_and_load_data('data/exoTrain.csv')
test_data = read_and_load_data('data/exoTest.csv')
# Load datasets
# train_data = load_data(TRAIN_FILE_LINK)
# test_data = load_data(TEST_FILE_LINK)
...
tab1, tab2, tab3 = st.tabs(
    ["Home", "Exploratory Data Analysis", "Model Training and Evaluation"]
)
...
with tab1:
    ...
    with col2:
        ...
        try:
            ...
            st.error(
                f"An error occurred when loading the historical discovery image: {e}"
            )
...
with tab3:
    ...
    model_selection = st.selectbox(
        "Choose a model to train:", options=list(model_options.keys())
    )
    ...
    if st.button("Train and Evaluate 👟"):
        ...
        with col1:
            ...
            )  # Transform the report dictionary to a DataFrame and transpose it for better viewing

        with col2:
            st.write(f"#### {model_selection} Confusion Matrix")
            fig, ax = plt.subplots(
                figsize=(8, 6)
            )  # Sizing the plot to make it more readable
